...
commands.push({
  ...
  async execute(interaction) {
    ...
    if (!guildData) throw new Error("guild");

    let currentMode = guildData.currentMode as any as keyof typeof modes;

    let nextMode = modeNameToInt(currentMode) as number + 1;

    if (nextMode > 4) {
      nextMode = 0;
    }

    currentMode = intToModeName(nextMode) as any as keyof typeof modes;

    // increment the current mode by 1
    const modeMaps = modes[currentMode];

    // get the map rotation for the current mode
    let maps: string[] = [];
    for (let i = 0; i < 3; i++) {
      const randomIndex = Math.floor(
        Math.random() * Object.keys(modeMaps).length,
      );

      // check if the map is already in the array
      if (maps.includes(Object.values(modeMaps)[randomIndex])) {
        i--;
        continue;
      }

      maps.push(Object.values(modeMaps)[randomIndex]);
    }
